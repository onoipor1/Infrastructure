...
Expand-Archive -LiteralPath "$(System.DefaultWorkingDirectory)\Azure\Orchestrator\20.10.1\ps_utils.zip" -DestinationPath . -Force
Write-Output "$(Get-Date) Unzipping AZModules..."
Expand-Archive -LiteralPath "$(System.DefaultWorkingDirectory)\Azure\Orchestrator\20.10.1\AzModules.zip" -DestinationPath . -Force
Write-Output "$(Get-Date) Unzip done."

Write-Output "$(Get-Date) Importing AzureRM modules..."
$env:PSModulePath += ";$(Resolve-Path -Path $(System.DefaultWorkingDirectory)\Azure\Orchestrator\20.10.1\AzModules)"
...
Import-Module ([System.IO.Path]::GetFullPath((Join-Path (Get-Location) "$(System.DefaultWorkingDirectory)\Azure\Orchestrator\20.10.1\ps_utils\CloudDeploymentUtils.ps1"))) -Global -Force
