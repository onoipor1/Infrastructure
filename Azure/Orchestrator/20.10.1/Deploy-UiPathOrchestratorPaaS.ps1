...
function PublishIdentityServer {
    ...
    d:/a/1/s/Azure/Orchestrator/20.10.1/Publish-IdentityServer.ps1 -action Deploy -azureSubscriptionId $azureSubscriptionId -azureAccountTenantId $azureTenantId -package '$(Build.ArtifactsDirectory)\BlobFile\UiPath.IdentityServer.Web.zip' -cliPackage ${env:IDENTITYCLIMIGRATOR} -stopApplicationBeforePublish -resourceGroupName $resourceGroupName -appServiceName $appServiceNameIdentity -orchestratorUrl $orchestratorUrl -noAzureAuthentication -unattended
...
function PublishWebhooks {
    ...
    d:/a/1/s/Azure/Orchestrator/20.10.1/Publish-Webhooks.ps1 -action "Deploy" -azureSubscriptionId $azureSubscriptionId -appServiceName $appServiceNameWebhooks -resourceGroupName $resourceGroupName -package '$(Build.ArtifactsDirectory)\BlobFile\UiPath.WebhookService.Web.zip' -stopApplicationBeforePublish -noAzureAuthentication
